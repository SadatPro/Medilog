...
import { IconCalendar, IconDocument, IconPrescription } from './icons'; 
                    ...
                    {activeTab === 'prescriptions' && (
                        <div className="space-y-3 animate-fade-in">
                            {sortedPrescriptions.length > 0 ? (
                                ...
                                <p className="text-sm text-gray-500 text-center py-8">{t('noPrescriptions')}</p>
                            )}
                        </div>
                    )}
                    {activeTab === 'reports' && (
                        <div className="animate-fade-in">
                            <div className="space-y-3">
                                {paginatedReports.length > 0 ? (
                                    ...
                                    <p className="text-sm text-gray-500 text-center py-8">{t('noRecords')}</p>
                                )}
                            </div>
                            {totalPages > 1 && (
                                <div className="mt-4 flex justify-between items-center">
                                    <button onClick={() => setCurrentPage(p => p - 1)} disabled={currentPage === 1} className="px-3 py-1 border border-gray-700 rounded-md hover:bg-gray-800 transition-colors text-xs font-plex-mono disabled:opacity-50">{t('previousPage')}</button>
                                    <span className="text-xs font-plex-mono text-gray-400">Page {currentPage} of {totalPages}</span>
                                    <button onClick={() => setCurrentPage(p => p + 1)} disabled={currentPage === totalPages} className="px-3 py-1 border border-gray-700 rounded-md hover:bg-gray-800 transition-colors text-xs font-plex-mono disabled:opacity-50">{t('nextPage')}</button>
                                </div>
                            )}
                        </div>
                    )}
