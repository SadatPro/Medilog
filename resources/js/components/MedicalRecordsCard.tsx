...
export const MedicalRecordsCard: React.FC<MedicalRecordsCardProps> = ({ patient, userType, onUpdate }) => {
        ...
        <>
            <div className="p-6 card-bg rounded-lg shadow-xl">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="font-plex-mono text-xl text-white">{t('medicalRecords')}</h2>
                    {userType === 'patient' && (
                ...
                {records.length === 0 ? (
                    <p className="text-sm text-gray-500 font-plex-mono">{t('noRecords')}</p>
                ) : (
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 font-plex-mono">
                                            ...
                                            {r.type} &bull; {new Date(r.date).toLocaleDateString('en-GB')}
                                        </p>
                                    </div>
                                </div>
            ...
            {isAddModalOpen && <AddRecordModal patient={patient} onClose={() => setIsAddModalOpen(false)} onUpdate={onUpdate!} />}
